import doorItem from "src/resources/images/door_item.png";
import doorDefault from "src/resources/images/door_default.png";

const mockData = [
  {
    id: 1,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: true,
        main_lock: true,
        additional_lock: false,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2000,
        height: 800,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorDefault,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2120,
        height: 100,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 1111",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2125,
        height: 222,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 10,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 11,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 12,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 13,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 14,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
  {
    id: 2,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
    ],
  },
  {
    id: 3,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
  {
    id: 4,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
  {
    id: 5,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
  {
    id: 6,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
  {
    id: 7,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
  {
    id: 8,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
  {
    id: 9,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
  {
    id: 10,
    title: "Дверь Гранит Лира 001",
    image_entry: doorDefault,
    image_exit: doorDefault,
    description:
      "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
    articles: [
      {
        id: 1,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 2,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 3,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 4,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 5,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 6,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 7,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 8,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
      {
        id: 9,
        title: "Арт. 0989",
        description:
          "Lorem ipsum dolor sit amet consectetur. Fringilla justo et sit duis pretium. Amet morbi purus donec pharetra vulputate velit. Non mauris egestas congue nullam",
        width: 2100,
        height: 700,
        opening_direction: false,
        main_lock: true,
        additional_lock: true,
        door_leaf_thickness: 150,
        sealing_contours: 4,
        color: "RAL 1334",
        target: "Входная",
        mirror: true,
        price: 13567,
        price_without_discount: 27134,
        discount: 50,
        in_stock: 456,
        images: [
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
          {
            url: doorItem,
          },
        ],
      },
    ],
  },
];

export default mockData;
